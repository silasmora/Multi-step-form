...
export const StepTwo = () => {
  ...
  const [selectedBilling, setSelectedBilling] = useState(formData.step2?.selectedBilling || 'monthly')

  {/*This is so that the toggle retains previous chosen data if user goes back to step2 */}
  const [enabled, setEnabled] = useState(formData.step2?.selectedBilling === 'yearly' ? true : false)
  ...
  return (
    <form onSubmit={handleSubmit} className='w-[70%] md:h-[568px] md:px-10'>
      ...
      <div className='mx-4 py-8 px-6 bg-white rounded-lg absolute top-28 left-0 right-0 md:relative md:top-0 md:mx-0 md:h-[80%]'>
        ...
        <h1 className='text-marineBlue font-bold text-3xl lg:text-4xl'>Select your plan</h1>
        <p className='pt-4 text-coolGray text-lg'>You have the option of monthly or yearly billing</p>
        ...
        <div className='flex justify-around bg-magnolia/75 rounded-lg mt-8 py-4 px-8 text-coolGray font-medium sm:px-24 md:px-0 md:mt-2 lg:mt-8'>

          <p className={`text-${selectedBilling === 'yearly' ? '' : 'marineBlue'}`}>Monthly</p>
